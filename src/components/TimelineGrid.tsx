...
export function TimelineGrid({ rows }: Props) {
  ...
  const bandCell = `${stickyCell} py-1 text-xs font-semibold`;
  ...
  const summaryRow = (label: string, pick: (r: MonthRow) => number, highlightNegative = false) => (
    <tr className="border-t border-line bg-surface-muted font-medium">
      <th scope="row" className={`${stickyCell} py-1`}>{label}</th>
      {rows.map((r) => {
        const v = pick(r);
        return (
          <td key={r.month} className={`${numCell} ${highlightNegative && v < 0 ? 'bg-loss-soft text-loss' : ''}`}>
            {money(v)}
          </td>
        );
      })}
  ...
  return (
    <section className={card}>
      <h2 className={`${sectionTitle} px-4 pt-4 pb-2`}>{t('gridTitle')}</h2>
      <div className="overflow-x-auto border-t border-line">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-accent-soft text-accent">
              <th scope="col" className={`${stickyCell} bg-accent-soft py-2 text-xs font-semibold`}>{t('item')}</th>
              {rows.map((r) => (
